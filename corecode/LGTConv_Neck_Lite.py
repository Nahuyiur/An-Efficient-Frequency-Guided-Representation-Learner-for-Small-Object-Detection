...
class LGTConvNeckLite(nn.Module):
    """
    轻量化LGT Neck模块
    ...
    使用方式：
    在YAML中：
    - [6, 1, LGTConvNeckLite, [128]]  # 处理Ci
    - [[-1, 11], 1, Concat, [1]]      # 与Pi+1融合
    """
    def __init__(self, c1, c2=None, kernel_size=5, num_orientations=2, num_scales=1):
        super(LGTConvNeckLite, self).__init__()
        ...
        self.loggabor_filter = LogGaborFilterLite(
...
class LGTConvNeckLiteV2(nn.Module):
    """
    ...
    """
    def __init__(self, c1, c2=None, kernel_size=5, num_orientations=2, num_scales=1):
        super(LGTConvNeckLiteV2, self).__init__()
...
class LGTConvNeckUltraLite(nn.Module):
    """
    ...
    def __init__(self, c1, c2=None, kernel_size=5):
        super(LGTConvNeckUltraLite, self).__init__()
        
        if c2 is None:
            c2 = c1
            
        # 简化：只用1个方向1个尺度
        self.loggabor_filter = LogGaborFilterLite(c1, kernel_size, num_orientations=1, num_scales=1)
...
# 导出模块
__all__ = [
    'LGTConvNeckLite',      # 推荐使用：保留核心功能，移除冗余
    'LGTConvNeckLiteV2',    # 更轻量：移除可学习权重
    'LGTConvNeckUltraLite', # 极致轻量：1方向1尺度
    'LogGaborFilterLite',   # 轻量Log-Gabor滤波器
]
