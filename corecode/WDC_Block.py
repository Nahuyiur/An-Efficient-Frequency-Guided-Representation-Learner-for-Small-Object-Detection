...
class WDC_Block(nn.Module):
    """
    Wavelet Difference Convolution Block
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):
        ...
        # --- 低频分支（LL）：使用 RepCDC 增强边缘感知 ---
        self.cdc_ll = nn.Sequential(
            RepCDC(c_, c_, kernel_size=3, padding=1),
            nn.BatchNorm2d(c_),
            nn.SiLU()
        )
...
class C3k2_WDC(nn.Module):
    """
    C3k2 模块，使用 WDC_Block 作为 Bottleneck
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        # 使用 WDC_Block 替换原始 Bottleneck
        if c3k:
            # 如果要求使用 C3k，我们仍然使用 WDC_Block（因为 C3k 也是 Bottleneck 的变种）
            self.m = nn.ModuleList(
                WDC_Block(self.c, self.c, shortcut, g, e=1.0) for _ in range(n)
            )
        else:
            self.m = nn.ModuleList(
                WDC_Block(self.c, self.c, shortcut, g, e=1.0) for _ in range(n)
...
if __name__ == "__main__":
    ...
    # 测试 RepCDC
    print("\n测试 RepCDC...")
    cdc = RepCDC(64, 64, kernel_size=3, padding=1)
    out_cdc = cdc(x)
    print(f"RepCDC 输出形状: {out_cdc.shape}")
    
    # 测试切换到部署模式
    cdc.switch_to_deploy()
    out_cdc_deploy = cdc(x)
    print(f"RepCDC (部署模式) 输出形状: {out_cdc_deploy.shape}")
    print(f"输出差异: {torch.abs(out_cdc - out_cdc_deploy).max().item():.6f}")

    # 测试 WDC_Block
    print("\n测试 WDC_Block...")
    wdc = WDC_Block(c1=64, c2=64, shortcut=True)
    out_wdc = wdc(x)
    print(f"WDC_Block 输出形状: {out_wdc.shape}")

    # 测试 C3k2_WDC
    print("\n测试 C3k2_WDC...")
    c3k2_wdc = C3k2_WDC(c1=64, c2=128, n=2, shortcut=True)
    out_c3k2 = c3k2_wdc(x)
    print(f"C3k2_WDC 输出形状: {out_c3k2.shape}")
    ...
    print("\n参数量统计:")
    print(f"RepCDC: {count_parameters(cdc):,} 参数")
    print(f"WDC_Block: {count_parameters(wdc):,} 参数")
    print(f"C3k2_WDC: {count_parameters(c3k2_wdc):,} 参数")
