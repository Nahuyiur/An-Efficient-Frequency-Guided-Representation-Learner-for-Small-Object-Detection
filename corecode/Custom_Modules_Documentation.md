...
本目录包含的自定义实现：
- `WDC_Block.py`
- `LGTConv_Neck_Lite.py`
- `LGTConv_NeckLite_WTConv.py`
- `WTConv_original.py`
- `Detect_LSDECD_DWTP2GateLite.py`
- `yolo11s-head2-LSDECD-DWTP2GateLite-LGTConv-Neck-Lite-WTConv-Internal-WDC.yaml`
...
### 4.1 `LogGaborFilterLite`（Log-Gabor 频域滤波器组，轻量实现）

**文件**：`LGTConv_Neck_Lite.py` 与 `LGTConv_NeckLite_WTConv.py`（两处都实现了一份同名类，逻辑一致）
...
### 4.2 `LGTConvNeckLite`（Log-Gabor 高频增强 Neck 模块，带可学习方向/尺度加权）
...
### 4.4 `WTConv2d_original`（Wavelet Transform Convolution，原始实现）

**文件**：`WTConv_original.py`
...
### 4.5 `LGTConvNeckLite_WT`（LGT Neck Lite + WTConv 的高频卷积替换）

**文件**：`LGTConv_NeckLite_WTConv.py`
...
### 4.7 `WDC_Block`（Wavelet Difference Convolution Block）
...
### 4.8 `C3k2_WDC`（将 YOLO 的 C3k2 Bottleneck 替换为 WDC_Block）
...
### 4.9 `Detect_LSDECD_DWTP2GateLite`（改造检测头：DEConv + P2 高频门控 + DFL）

**文件**：`Detect_LSDECD_DWTP2GateLite.py`
